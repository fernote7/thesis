...
% Everything below added by CII
  ...
  process}. These processes generally have a continuous paths. We must
  carefully define what the term \emph{random}, means and the best way to
  begin doing so is to precisely define a probability space:
  ...
  {[}3,4{]}.\footnote{More can be found on {[}3--5{]}.}
  ...
  calculus in allowing to evaluate integrals. Itô's lemma has an extra
  term not present in the conventional calculus theorem that is due to the
  non-smoothness characteristics of Brownian motion paths. If we write
  \begin{align}
  ...
  increments and its solution is presented below{[}11{]}:
  \begin{align}
  ...
  whose values depend on the values of an underlying asset. {[}12--14{]}
  \begin{itemize}
  \item
  ...
  the risk-neutral measure with the following dynamics {[}3,16{]}:
  \begin{align}
  dS_{t} = (r-q)S_td_t+\sigma S_t dW_t
  \end{align}
  \noindent
  Where \(dW_t\) is a Wiener process {[}14,16{]}, \(r\) is the risk free
  ...
  moves and large moves by large moves. {[}3,12{]}
  ...
  {[}10{]} R.S. Tsay, Analysis of financial time series, John Wiley \&
  Sons, 2005.
  ...
  {[}14{]} F. Black, M. Scholes, The pricing of options and corporate
