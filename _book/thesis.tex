...
% Everything below added by CII
  ...
  \end{split}
  \end{align}
  The second equation, as previously stated, is the CIR model equation.
  The first equation describes the asset price process. \(\mu\) is the
  asset's rate of return, \(dB_t\) and \(dW_t\) are two correlated wiener
  processes with correlation coefficient of \(\rho\).
  
  \subsection{Correlated Brownian
  Motions}\label{correlated-brownian-motions}
  
  Let \(W^1\) and \(W^2\) be two \emph{independent} Brownian motions. Let
  \(-1 \leq \rho \leq 1\) be a given number. For \(0 \leq t \leq T\)
