...
% Everything below added by CII
  ...
  \begin{align}
  V_{t+1} = V_{t,1} + \kappa (\theta - V_{t,2}) + \sigma \sqrt{V_{t,3}} Z_v \text{\footnote{teste}}
  \end{align}
  \noindent
  ...
  \toprule
  Scheme & \(V_{t,1}\) & \(V_{t,2}\) & \(V_{t,3}\)\tabularnewline
  \midrule
  \endhead
  Reflection & \(\mid V \mid\) & \(\mid V \mid\) &
  \(\mid V \mid\)\tabularnewline
  Partial Truncation & \(V\) & \(V\) & \(V^+\)\tabularnewline
  Full Truncation & \(V\) & \(V^+\) & \(V^+\)\tabularnewline
  \bottomrule
  \end{longtable}
