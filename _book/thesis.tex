...
% Everything below added by CII
  ...
  {[}2{]} Another technique of particular relevance when the random
  ...
  we can rewrite it in the form {[}6--11{]}:
  \begin{align} \label{sde1}
  ...
  \ref{tab:box-calc} multiplication table {[}3{]}.
  ...
  random process (S) defined as follows {[}3{]}:
  \begin{align}
  ...
  increments and its solution is presented below{[}16{]}:
  \begin{align}
  ...
  the risk-neutral measure with the following dynamics {[}5,17{]}:
  \begin{align}
  dS_{t} = (r-q)S_td_t+\sigma S_t dW_t
  \end{align}
  \noindent
  Where \(dW_t\) is a Wiener process {[}1,17{]}, \(r\) is the risk free
  ...
  1993 Steven Heston's paper {[}19{]} and is described below {[}22{]}:
  \begin{align}
  ...
  a strike price \(K\), that expires at time \(T\) {[}22{]}:
  \begin{align} 
  \label{eq:cfheston}
  \begin{split}
  ...
  We present here the Euler Scheme - Full Truncation algorithm {[}21{]}
  along with some insights on how it was implemented in the R programming
  language {[}23{]}. The Euler discretization brings approximation paths
  ...
  {[}2{]} T.C. Gard, Introduction to stochastic differential equations, M.
  Dekker, 1988.
  ...
  {[}11{]} K. Itô, The brownian motion and tensor fields on riemannian
  ...
  {[}22{]} R. Dunn, P. Hauser, T. Seibold, H. Gong, Estimating option
