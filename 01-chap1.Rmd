...
This ``noise''  $dW_t$ is a *random* Wiener process (which will be clarified below) and $x_0$ is our initial value. 

The $g(t, x_t)$ part of the SDE is often referred as *diffusion coefficient*, these usually have a continuous paths. Before moving on, we must carefully define what the term *random* means and the best way to begin doing so is to precisely define a probability space:
...
The Itô integral can, as the Riemann integral, be approximated by a finite sum. Also, it has a definition as a certain limit. Itô's lemma \@ref(lem:itolemma) plays the same role as the fundamental theorem of calculus in allowing to evaluate integrals. It is the formal definition and presents an extra term not encountered in the conventional calculus theorem that is due to the non-smoothness characteristics of Brownian motion paths. It is possible, though, to define the integral in a less rigorous way: 
...
```{definition, name="Intrinsic value"}
