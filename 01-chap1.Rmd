...
We know that convergence is an important feature to a numerical method and studies have found not all time discrete possible approximations of an SDE converge in a useful sense to the solution process as the step size adopted tends toward zero [@clements1973well; @clark1980maximum]. Moreover, particularly for SDE's, some of the more rapidly convergent methods available for ODE's ^[Ordinary differential equations.] do not work, such as higher order Runge-Kutta methods. One of the methods that do work for ODE's and SDE's is the Euler method. Figure \@ref(fig:euler) shows an example of an implementation for the Newton's cooling law with timestep of 2 seconds compared to its analytical solution. 
...
It is clear that for ordinary differential equations the Euler method produces good enough approximations and since we not always have an analytical solution to compare to and that its computational cost is comparatively low, this method became very popular. We proceed to the SDE's. Consider the equation:
