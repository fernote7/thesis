...
This ``noise''  $dW_t$ is a *random* Wiener process time derivative (which will be clarified below) and $X_0$ is ou initial value. 

The solution to this class of SDE is often referred as a *diffusion process*. These processes generally have a continuous paths. We must carefully define what the term *random*, means and the best way to begin doing so is to precisely define a probability space:
...
In 1923, the Wiener process was coined in honor of Norbert Wiener mathematical proof of existence of the brownian motion and stating its properties. We start by defining what is a probability space [@helgadottir2016option; @evans] ^[More can be found on [@helgadottir2016option; @evans; @rosenthal].].
...
<!-- Let $S_{t}$ be a real-valued stochastic process that satisfies [@tong2012option; @evans; @steele2012stochastic; @ito1951; @ito1962; @rosenthal]: -->
...
The Itô integral can, as the Riemann integral, be approximated by a finite sum. Also, it has a definition as a certain limit. Itô's lemma \@ref(itolemma) plays the same role as the fundamental theorem of calculus in allowing to evaluate integrals. Itô's lemma has an extra term not present in the conventional calculus theorem that is due to the non-smoothness characteristics of Brownian motion paths. If we write 
...
\noindent
with the usual notions $t_k = k\Delta t$, and $\Delta W_k = W(t_{k+1})-W(t_k))$. If the limit
exists, then the Ito integral is:
...
Before introducing the Black-Schole model...
